...
class Friends:
    def __init__(self, connections):
        self.frnds=list(connections)

    def add(self, connection):
        if connection in self.frnds:
                return False
        self.frnds.append(connection)
        return True

    def remove(self, connection):
        if connection in self.frnds:
           self.frnds.remove(connection)
           return True
        return False 

    def names(self):
        return set().union(*self.frnds)

    def connected(self, name):
        s=set()
        for f1,f2 in self.frnds:
            if(f1==name):
                s.add(f2)
            if(f2==name):
                s.add(f1)
