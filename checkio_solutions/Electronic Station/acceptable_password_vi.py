...
def is_acceptable_password(password: str) -> bool:
    # your code here
    return True if ((len(password)>6 and [True  for c in password if c.isdigit()] and [True  for c in password if c.isalpha()]
                    ) or len(password)>9) and 'password' not in password.lower() and len(set(password))>=3 else False
