...
def checkio(data):
    ...
    for word in data:
        for i in range(len(word)):
            [d[c].add(word[i]) for c in word[i+1:] if c!=word[i]]
    
    chars = set(c for s in data for c in s)
    
    result=[]
    
    while chars:
        root = sorted( c for c in chars if len(d[c])==0)[0]
        result.append(root)
        chars.difference_update(root)
        [d[c].discard(root) for c in chars]
